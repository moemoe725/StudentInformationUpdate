...
public class SubjectController {
	...
	@RequestMapping(value="/delete2.htm/{id}",method=RequestMethod.GET)
	public String deleteSubject(@PathVariable(value="id")Integer id) {
		subjectService.deleteSubject(id);
		return "redirect:/cindex.htm";
	}
	@RequestMapping(value="/update2.htm/{id}",method=RequestMethod.GET)
	public String updateSubject(@PathVariable(value="id")Integer id,Model model) {
		model.addAttribute("subDto",subjectService.getSubjectId(id));
		model.addAttribute("classesList", subjectService.getClassesList());
		return "new_subject";
	}
}
