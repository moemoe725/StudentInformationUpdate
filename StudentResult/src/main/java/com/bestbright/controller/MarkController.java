...
public class MarkController{
	...
	public @ResponseBody List<StudentDto> getStuList(@PathVariable(value="classesId")Integer id){
		List<StudentDto> stuList=studentService.getStudentListByClassId(id);
		return stuList;
	}
	...
	public String deleteMark(@PathVariable(value="id")Integer id) {
		markService.deleteMark(id);
		return "redirect:/menu.htm";
	}
	@RequestMapping(value="/update1.htm/{id}",method=RequestMethod.GET)
	public String updateMark(@PathVariable(value="id")Integer id,Model model) {
		model.addAttribute("markDto",markService.getMarkId(id));
		model.addAttribute("studentList",markService.getStudentList());
		model.addAttribute("classesList",markService.getClassesList());
		model.addAttribute("semisterList",markService.getSemisterList());
		model.addAttribute("subjectList",markService.getSubjectList());
	...
	@RequestMapping(value="/showTotalMark.htm",method=RequestMethod.GET)
	public String showTotalMark(Model model) {
		return "totalMark";
	}
}
